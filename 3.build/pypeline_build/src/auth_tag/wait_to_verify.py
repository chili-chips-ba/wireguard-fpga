...
class wait_to_verify_out_t(NamedTuple):
    ...
    verify_bit_ready: uint1_t
    axis_out: axis128_t
    is_verified_out: uint1_t


@hw_func
def wait_to_verify(
    axis_in: axis128_t,
    verify_bit: uint1_stream_t,
    ...
    if state == wait_to_verify_state_t.WAIT_TO_VERIFY_BIT:
        ...
        o.axis_out.valid = verify_fifo_out.valid
    ...
    fifo_result = verify_fifo_func(verify_fifo_out_ready_s, verify_fifo_in_s)
    verify_fifo_out = fifo_result.out_stream
    verify_fifo_in_ready = fifo_result.in_ready
