...
"""chacha20 instance for the decrypt half of the shared design: same globally
visible wires + FSM MAIN as chacha20_decrypt.py, but with no pipeline of its
own — the FSM talks to the shared pipeline's decrypt-side wires in
chacha20_pipeline_shared.py instead.

Pypeline port of ../pipelinec_build/src/chacha20/chacha20.c with
CHACHA_INST=chacha20_decrypt + CHACHA_EXCLUDES_PIPELINE
(wire names elaborate as chacha20_decrypt_shared_<wire>).
"""
...
def chacha20_decrypt_shared():
    fsm_out = chacha20.chacha20_fsm(
        ...
        poly_key_ready,
        axis_out_ready,
        chacha20_pipeline_shared.decrypt_pipeline_in_ready,
        chacha20_pipeline_shared.decrypt_pipeline_out,
    )
    axis_in_ready = fsm_out.ready_for_axis_in
    poly_key = fsm_out.poly_key
    axis_out = fsm_out.axis
    chacha20_pipeline_shared.decrypt_pipeline_in = fsm_out.to_pipeline
    chacha20_pipeline_shared.decrypt_pipeline_out_ready = fsm_out.ready_for_from_pipeline
