...
ot = ComPort.write(struct.pack('B', int(0x0C)))          #ADDR0
print("ADDR0: ", hex(0x0C))

ot = ComPort.write(struct.pack('B', int(0x08)))          #ADDR1
print("ADDR1: ", hex(0x08))

ot = ComPort.write(struct.pack('B', int(0x00)))          #ADDR2
print("ADDR2: ", hex(0x00))

ot = ComPort.write(struct.pack('B', int(0x10)))          #ADDR3
...
checksum = 0x0C + 0x08 + 0x00 + 0x10 + int.from_bytes(it1, byteorder='big') + int.from_bytes(it2, byteorder='big') + int.from_bytes(it3, byteorder='big') + int.from_bytes(it4, byteorder='big');
